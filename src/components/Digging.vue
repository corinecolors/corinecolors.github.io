<template>
  <div class="digging" ref="digging">
      ...
      <canvas :id="`canvas${data.i}B`" class="b" :style="`${Asolved ? `opacity: 1;` : ``}`"/>
      <canvas :id="`canvas${data.i}A`" class="a" :style="`${Asolved ? `opacity: 0; pointer-events: none;` : ``}`"/>
      ...
      <span @mousedown="handlePlayButton">
        <Playbutton :fill="$cms.textField(this.data.data.playbuttom_color)" :style="`${Asolved ? `opacity: .75; cursor: pointer;` : `pointer-events: none`}`"/>
      </span>
      <canvas :id="`canvas${data.i}AA`" class="aa" :style="`${AAsolved ? `opacity: 0; pointer-events: none;` : ``}`"/>
  </div>
</template>
...
export default {
  ...
  props: {
      ...
      image: {
          type: String,
          default: 'https://cloud.githubusercontent.com/assets/4652816/12771961/5341c3c4-ca68-11e5-844c-f659831d9c00.jpg'
      },
      activei: {
          type: Number,
          default: 0
  ...
  methods: {
      ...
      calcArray(old) {
        ...
        //DETERMINE IF SCRUBBED ENOUGH
          if (this.whArray.length < 5 && this.active === document.getElementById(`canvas${this.data.i}AA`)) {
              console.log("width and height", this.width, this.height)
              this.setArray({width: this.width, height: this.height});
              this.AAsolved = true;
          } 

        if (this.whArray.length < 5 && this.active === document.getElementById(`canvas${this.data.i}A`)) {
            this.Asolved = true;
        } 
